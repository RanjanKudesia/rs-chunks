...
fn assemble(glyphs: &mut Vec<&Glyph>, baseline: f32, turn: u8) -> Line {
    ...
    for g in glyphs.iter() {
        ...
        //
        // This once had a second rule beside it, raising the threshold on a
        // size *drop*, to stop `VERY DEEP` coming out as `V ERY D EEP`. That
        // was a workaround: the real cause was that the base-14 fonts carried
        // no widths, so every glyph advanced half an em and opened phantom gaps
        // (see `base14.rs`, TECH_DEBT #92/#93). With real metrics the gap after
        // the `V` is 1.0 pt against a 3.4 pt threshold, and the rule never
        // fired again — so it was removed rather than left to imply that
        // small caps need special handling.
