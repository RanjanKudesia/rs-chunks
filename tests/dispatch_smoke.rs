...
fn get_chunks_default_mode_every_family() {
    let mut checked = 0;
    for ext in FAMILIES {
        let Some(path) = first_with_ext(ext) else {
            continue;
        };
    ...
    assert!(
        checked >= 30,
        "expected to exercise most families, only did {checked}"
    );
}
...
fn get_markdown_smoke() {
    for ext in [
        "md", "html", "csv", "docx", "pptx", "xlsx", "pdf", "eml", "odt", "rtf", "doc",
    ] {
        let Some(path) = first_with_ext(ext) else {
            continue;
        };
...
fn bytes_api_matches_path_api() {
    ...
    for ext in ["md", "docx", "csv", "xlsx"] {
        ...
        assert_eq!(
            md_path, md_bytes,
            "bytes markdown != path markdown for {ext}"
        );
    }
}
