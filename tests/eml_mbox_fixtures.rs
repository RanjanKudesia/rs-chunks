...
fn eml_corpus_chunks_cleanly() {
    ...
    assert!(files.len() >= 10, "expected >= 10 .eml fixtures, found {}", files.len());
    ...
    // just not-crash.
    assert!(ok * 2 >= files.len(), "only {ok}/{} .eml fixtures chunked Ok", files.len());
    assert!(total_chunks > 0, "no chunks produced from the whole .eml corpus");
}
...
fn eml_markdown_and_bytes_roundtrip() {
    ...
    assert!(md_nonempty > 0, "no .eml fixture produced non-empty markdown");
}

#[test]
fn mbox_corpus_chunks_cleanly() {
    let files = fixtures("mbox", "mbox");
    assert!(files.len() >= 5, "expected >= 5 .mbox fixtures, found {}", files.len());
    ...
    assert!(total_chunks > 0, "no chunks produced from the whole .mbox corpus");
}
