...
fn first_with_ext(ext: &str) -> Option<PathBuf> {
    fn walk(dir: &Path, ext: &str) -> Option<PathBuf> {
        ...
        for p in &entries {
            if p.is_dir() {
                if let Some(f) = walk(p, ext) { return Some(f); }
    ...
    walk(&Path::new(env!("CARGO_MANIFEST_DIR")).join("..").join("test_files"), ext)
}
...
fn chunk_with_images_heavy_formats() {
    if let Some(p) = first_with_ext("docx") {
        let (c, i) = formats::docx::chunk_with_images(p.to_str().unwrap(), "default", 3, 1, 3, 15).unwrap();
        assert_well_formed(&c, &i);
    }
    if let Some(p) = first_with_ext("pptx") {
        let (c, i) = formats::pptx::chunk_with_images(p.to_str().unwrap(), "default", 3, 1, 3, 15).unwrap();
        assert_well_formed(&c, &i);
    }
    if let Some(p) = first_with_ext("xlsx") {
        let (c, i) = formats::xlsx::chunk_with_images(p.to_str().unwrap(), "row", 1, 3, 1, true, Vec::new(), true, 2000).unwrap();
        assert_well_formed(&c, &i);
    }
    if let Some(p) = first_with_ext("doc") {
        let (c, i) = formats::doc::chunk_with_images(p.to_str().unwrap(), "default", 3, 1, 3, 15).unwrap();
        assert_well_formed(&c, &i);
    }
    if let Some(p) = first_with_ext("ppt") {
        let (c, i) = formats::ppt::chunk_with_images(p.to_str().unwrap(), "default", 3, 1, 3, 15).unwrap();
        assert_well_formed(&c, &i);
    }
}
...
fn to_markdown_with_images_heavy_formats() {
    let cases: &[(&str, fn(&str) -> chunks_rs::Result<(String, Vec<(String, Vec<u8>)>)>)] = &[
